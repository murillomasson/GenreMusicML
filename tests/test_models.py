...
def test_spotify_service_initialization(mocker, spotify_service):
    ...
    mocker.patch.object(spotify_service.sp, 'recommendations', return_value={'tracks': []})
    
    # Verifica se a instância foi criada corretamente e se a autenticação está funcionando
...
def test_get_recommendations(mocker, spotify_service):
    ...
    mocker.patch.object(spotify_service.sp, 'recommendations', return_value=mock_recommendations)
...
def test_get_audio_features(mocker, spotify_service):
    ...
    mocker.patch.object(spotify_service.sp, 'audio_features', return_value=mock_features)
